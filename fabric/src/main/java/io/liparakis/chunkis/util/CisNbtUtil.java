...
public final class CisNbtUtil {
    ...
    public static NbtCompound extractStructureData(final NbtCompound root) {
        ...
        final NbtCompound structures = root.getCompound(STRUCTURES_KEY).orElse(null);
    ...
    public static NbtCompound extractPersistedStructureMetadata(final NbtCompound chunkMetadata) {
        ...
        if (chunkMetadata.contains(STRUCTURES_KEY)) {
            final NbtCompound structures = chunkMetadata.getCompound(STRUCTURES_KEY).orElse(null);
            return hasStructureData(structures) ? structures.copy() : null;
        }
    ...
    private static Boolean readSuppressInitialRepopulationFlag(final NbtCompound chunkMetadata) {
        ...
        final NbtCompound chunkisMetadata = chunkMetadata.getCompound(CHUNKIS_METADATA_KEY).orElse(null);
        if (chunkisMetadata == null) {
            return null;
        }
        ...
        return chunkisMetadata.getBoolean(SUPPRESS_INITIAL_REPOPULATION_KEY).orElse(null);
    }
    ...
    private static boolean hasChunkisDeltaMarker(final NbtCompound root) {
        ...
        return root.getCompound(CHUNKIS_DATA_KEY)
                .flatMap(chunkisData -> chunkisData.getBoolean(HAS_DELTA_KEY))
                .orElse(false);
    }
    ...
    private static boolean hasStructureData(final NbtCompound structures) {
        ...
        return structures.getCompound(STRUCTURE_STARTS_KEY).isPresent()
                || structures.getCompound(STRUCTURE_REFERENCES_KEY).isPresent();
    }
