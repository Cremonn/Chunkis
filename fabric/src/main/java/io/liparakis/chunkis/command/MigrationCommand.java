...
public final class MigrationCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(CommandManager.literal("chunkis").requires(source -> source.getPermissions().hasPermission(new Permission.Level(PermissionLevel.GAMEMASTERS))).then(CommandManager.literal("migrate").executes(MigrationCommand::runMigration)));
    }
    ...
    private static int runMigration(final CommandContext<ServerCommandSource> context) {
        ...
        source.sendFeedback(() -> Text.literal("§6[Chunkis] Starting CIS migration to v" + CisVersionMap.latestVersion() + "..."), true);
    ...
    private static void executeMigration(final ServerCommandSource source) {
        try {
            ...
            source.sendFeedback(() -> Text.literal("§a[Chunkis] CIS migration complete. Migrated " + finalTotal.migratedChunks() + " chunk(s), skipped " + finalTotal.skippedChunks() + ", failed " + finalTotal.failedChunks() + "."), true);

        } catch (final Exception e) {
            LOGGER.error("CIS migration failed", e);
            source.sendFeedback(() -> Text.literal("§c[Chunkis] CIS migration failed. Check server logs for details."), true);
        }
    }
    ...
    private static CisMigrationReport migrateWorld(final ServerWorld world, final ServerCommandSource source) {
        ...
        source.sendFeedback(() -> Text.literal("§8" + dimId + ": migrated " + report.migratedChunks() + ", skipped " + report.skippedChunks() + ", failed " + report.failedChunks()), false);
    ...
    private static CisMigrationReport merge(final CisMigrationReport left, final CisMigrationReport right) {
        return new CisMigrationReport(left.scannedChunks() + right.scannedChunks(), left.migratedChunks() + right.migratedChunks(), left.skippedChunks() + right.skippedChunks(), left.failedChunks() + right.failedChunks());
    }
}