 ...
 * <p>
 * This class implements a palette data structure commonly used for data
 * compression
 * in chunk storage systems. It maintains a one-to-one correspondence between
 * unique
 * objects and sequential integer IDs, allowing repeated objects to be
 * represented
 * by small integer references instead of storing the full object multiple
 * times.
 * </p>
 * <p>
 * The palette is particularly useful for BlockStates in Minecraft chunks,
 * where:
 * <ul>
 * <li>Many blocks in a chunk share the same state (e.g., multiple stone
 * blocks)</li>
 * <li>Storing a single BlockState reference plus many integer IDs is more
 * memory-efficient</li>
 * <li>Integer IDs can be packed into compact bit arrays for further
 * compression</li>
 * </ul>
 * </p>
 * <p>
 ...
 * <li>Not thread-safe - external synchronization required for concurrent
 * access</li>
 * </ul>
 * </p>
 * <p>
 ...
 * <ul>
 * <li>Initial capacity: 64 entries (tuned for typical chunk diversity)</li>
...
public class Palette<T> {
    ...
    private List<T> idToEntry = new ArrayList<>(INITIAL_CAPACITY);

    /** Reverse lookup mapping entries to their assigned IDs (Entry → ID) */
    private Object2IntMap<T> entryToId = new Object2IntOpenHashMap<>(INITIAL_CAPACITY);

    public Palette() {
        entryToId.defaultReturnValue(-1);
    }

    /**
     * Creates a palette with a size hint to avoid rehashing.
     *
     * @param expectedSize the expected number of unique entries
     */
    public Palette(int expectedSize) {
        this.idToEntry = new ArrayList<>(expectedSize);
        this.entryToId = new Object2IntOpenHashMap<>(expectedSize);
        entryToId.defaultReturnValue(-1);
    }

    /**
     * Retrieves the ID for the given entry, adding it to the palette if not
     * present.
     * <p>
     * This method ensures that each unique object gets a consistent ID. If the
     * object
     * is already in the palette, its existing ID is returned. Otherwise, a new ID
     * is
    ...
    public int getOrAdd(T entry) {
        ...
        int existingId = entryToId.getInt(entry);
        if (existingId != -1) {
            return existingId;
        }
     ...
     * {@code null} is returned instead of throwing an exception for performance
     * reasons.
     * </p>
     *
     * @param id the palette ID to look up
     * @return the entry associated with this ID, or {@code null} if the ID is
     *         invalid
     */
    public T get(int id) {
     ...
     * This method returns the internal list for performance reasons (avoiding
     * copies
